...
const CircularLinkedList_1 = __importDefault(require("../_datastructures/CircularLinkedList"));
const LoopArrow_1 = __importDefault(require("./LoopArrow"));
function Page() {
    const [cll] = (0, react_1.useState)(new CircularLinkedList_1.default());
    ...
    const renderNodes = () => {
        if (!cll.head)
            return <div>No nodes in the Circular Linked List.</div>;
        const nodes = [];
        ...
        return nodes.map((node, index) => (<div key={index} className="flex items-center justify-center relative" id={`node-${index}`}>
        ...
        {node.next !== cll.head ? (<div className="flex justify-center items-center text-3xl text-green-500">
            →
          </div>) : (<LoopArrow_1.default startId={`node-${index}`} endId="node-0"/>)}
      </div>));
    };
